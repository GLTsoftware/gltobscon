...
int main(int argc, char *argv[])  
{
 ...
 if(argc<2) usage(-1,"Insufficient number of arguments","At least azoff(arcseconds) required.");

        optCon = poptGetContext("azoff", argc, argv, optionsTable,0);
	...
	for(i=0;i<30;i++) {
	 ...
	 if(gotazoff!=1) usage(-2,"No offset specified","Az (arcseconds) is required .\n");
	...
	/* initializing ref. mem. */
      ...
      dsm_status=dsm_write(DSM_HOST, "DSM_COMMANDED_AZOFF_ARCSEC_D",&azoff);
         if(dsm_status != DSM_SUCCESS) {
                dsm_error_message(dsm_status,"dsm_write()");
                exit(1);
        }
