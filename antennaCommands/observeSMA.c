...
void checkResults(trackStatus *status, int ant) {
  if (status == NULL) {
    fprintf(stderr, "Null pointer returned by Track on acc%d\n", ant);
    return;
  } else if (status->result == STT_WARNING) {
    fprintf(stderr, "Received a warning (%d) from Track on acc%d\n", status->errorCode, ant);
    if (strlen(status->message) > 0)
      fprintf(stderr, "Returned message: \"%s\"\n", status->message);
  } else if (status->result == STT_ERROR) {
    fprintf(stderr, "Received an error (%d) from Track on acc%d\n", status->errorCode, ant);
    if (strlen(status->message) > 0)
      fprintf(stderr, "Returned message: \"%s\"\n", status->message);
  }
}
...
int main(int argc, char *argv[])  
{
  ...
  flagChange list;
  intValue rpcInt;
  CLIENT *antCl[SMAPOPT_MAX_ANTENNAS+1];
  CLIENT *cl = NULL;
  ...
  rpcInt.value = STT_RX_OFFSET_MIDDLE;
  for (i = 1; i <= 8; i++)
    if (antennaArray[i]) {
      char accName[5];
      
      sprintf(accName, "acc%d", i);
      if (!(antCl[i] = clnt_create(accName, TRACKPROG, TRACKVERS, "tcp"))) {
      clnt_pcreateerror("client create to acc");
	fprintf(stderr, "Could not connect to Track server thread on %s\n", accName);
	antCl[i] = NULL;
      } else {
	checkResults(set_rx_offset_mode_1(&rpcInt, antCl[i]), i);
      }
      /*
	rm_write(i, "RM_USE_RXA_POINTING_OFFSETS_S", &shortZero);
	if (rm_status != RM_SUCCESS) {
	rm_error_message(rm_status,"rm_write(RM_USE_RXA_POINTING_OFFSETS_S)");
	exit(1);
	}
      */
    }
  ...
  for (iant = 1; iant <= ANT_ARRAY_SZ; iant++) {
    observeParams newSource;
    lookupParams catSource;
	...
	trackStatus = 1;
      ...
      if (trackStatus == 1) {
	if (newSourceFlag) {
	  strncpy(newSource.name, Source, 34);
	  newSource.RA          = ra*M_PI/12.0;
	  newSource.Dec         = dec*M_PI/180.0;
	  newSource.epoch       = epoch;
	  newSource.pmRA        = (pmra*0.1/3600.0)*M_PI/180.0; /* Convert from mas per year to rad/century */
	  newSource.pmDec       = (pmdec*0.1/3600.0)*M_PI/180.0;
	  newSource.offset.dRA  = (rAOff/3600.0)*M_PI/180.0; /* Convert from arcsec to radians */
	  newSource.offset.dDec = (decOff/3600.0)*M_PI/180.0;
	  if (antCl[iant] != NULL)
	    checkResults(observe_1(&newSource, antCl[iant]), iant);

	} else {
	  strncpy(catSource.name, Source, 34);
	  catSource.offset.dRA  = (rAOff/3600.0)*M_PI/180.0;
	  catSource.offset.dDec = (decOff/3600.0)*M_PI/180.0;
	  if (antCl[iant] != NULL)
	    checkResults(observe_lookup_1(&catSource, antCl[iant]), iant);
