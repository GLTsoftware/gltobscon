...
int main(int argc, char *argv[])  {
	...
	time_t dsmtimestamp;
        int stowFlag=0; /* 0 for survival 1 for maintenance */

	 struct  poptOption optionsTable[] = {
                {"help",'h',POPT_ARG_NONE,0,'h'},
                {"maintenance",'m',POPT_ARG_NONE,0,'m'},
                {"survival",'s',POPT_ARG_NONE,0,'s'},
                {NULL,0,0,NULL,0}
        };
	...
	if (stowFlag==0) command_n[0]='7'; 
	if (stowFlag==1) command_n[0]='8'; 
        pmac_command_flag=0;
